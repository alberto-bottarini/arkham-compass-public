(function () {
  ...
  var slugs = window.__PAGE_SLUGS__ || { investigators: 'investigators', cards: 'cards' };
  var index = null;
  var activeIndex = -1;
  ...
  function investigatorLabel() { return i18n.investigator || 'Investigator'; }
  ...
  function render(results) {
    ...
    if (!results.length) {
      ...
      dropdown.innerHTML = results.slice(0, 12).map(function (r, i) {
        if (r.t === 'i') {
          return '<a class="search-item search-item--investigator" href="/' + lang + '/' + slugs.investigators + '/' + r.slug + '/" data-idx="' + i + '">'
            + '<span class="search-item__type">' + escHtml(investigatorLabel()) + '</span>'
